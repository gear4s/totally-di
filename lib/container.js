                ...
                case 'Object':
                    ...
                    for (var key in currentBinding.dependency) {
                        ...
                        if (currentBinding.dependency.hasOwnProperty(key)) {
                            ...
                            var constrName = self.__getConstructorName(currentBinding.dependency[key]);

                            // check if the value associated with the key contains '@inject:'
                            if (constrName == 'String' && constrName.length > 0) {

                                var nestedAliasIndex = currentBinding.dependency[key].indexOf('@inject:');

                                if (nestedAliasIndex > -1) {
                                    var nestedAlias = currentBinding.dependency[key].substr(nestedAliasIndex + 8);
                                    var nestedBinding = self.__getBinding(nestedAlias);
                                    currentBinding.dependency[key] = self.__createInstance(nestedBinding);
                                }
                            }
                        }
                    }
